  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main id="main-content">
        <section className="bg-[url('https://images.unsplash.com/photo-1541339907198-e08756dedf3f?q=80&w=1500&auto=format&fit=crop')] bg-cover bg-center bg-no-repeat relative h-screen flex items-center pt-16 lg:pt-0" aria-labelledby="hero-heading">
          <div className="absolute inset-0 bg-midnight/85 md:bg-gradient-to-br md:from-[#0B1F3A]/90 md:to-transparent"></div>

          <div className="relative z-10 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-4xl mx-auto">
              <h1 id="hero-heading" className="text-4xl sm:text-5xl lg:text-7xl font-serif font-bold tracking-tight mb-6 sm:mb-8 text-white leading-tight mt-12 md:mt-0">
                Your research, one link,{" "}
                <span className="text-warm">always up to date</span>
              </h1>
                      ...
                      <ul className="max-h-72 overflow-y-auto">
                        {searchResults.results.map((author, index) => (
                          <li
                            ...
                            className={`p-4 cursor-pointer border-b border-border last:border-b-0 hover:bg-muted/50 transition-colors ${index === selectedIndex ? 'bg-muted/70' : ''
                              }`}
              ...
              {features.map((feature, index) => (
                <Card key={index} className="group border bg-white shadow-sm hover:shadow-lg hover:border-primary/20 transition-all duration-300">
                  <CardHeader className="pb-2">
                    ...
                    <CardTitle className="text-xl font-serif text-midnight">{feature.title}</CardTitle>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="grid sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
              <Card className="border bg-gradient-to-br from-blue-50 to-white hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  ...
                  <h3 className="font-serif font-bold text-xl text-midnight mb-2">Email Signature</h3>
              ...
              <Card className="border bg-gradient-to-br from-green-50 to-white hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  ...
                  <h3 className="font-serif font-bold text-xl text-midnight mb-2">Grant Applications</h3>
                  <p className="text-sm text-muted-foreground">
              ...
              <Card className="border bg-gradient-to-br from-amber-50 to-white hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  <div className="text-3xl mb-3">🤝</div>
                  <h3 className="font-serif font-bold text-xl text-midnight mb-2">Collaboration</h3>
                  <p className="text-sm text-muted-foreground">
          ...
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-3">
              {faqs.map((faq, index) => (
                <div
                  key={index}
                  className={`bg-white rounded-lg shadow-sm border overflow-hidden transition-all ${openFaq === index ? 'border-l-4 border-l-primary' : 'border-border'
                    }`}
                >
                  <button
                    className={`w-full px-6 py-5 text-left flex items-center justify-between gap-4 ${openFaq === index ? 'bg-primary/5' : 'hover:bg-muted/50'
                      }`}
                    onClick={() => setOpenFaq(openFaq === index ? null : index)}
                    data-testid={`faq-question-${index}`}
                  >
                    <span className="font-medium text-foreground">{faq.question}</span>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${openFaq === index ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
                      }`}>
                      {openFaq === index ? (
